...
class FirstSFIConv(nn.Module):
	def __init__(
			self, in_channels, out_channels, kernel_size, alpha=0.5, stride=1, padding=1, dilation=1,
			groups=1, bias=False
			):
		...
		self.s2s = torch.nn.Conv2d(
				in_channels, out_channels - int(alpha * out_channels),
				kernel_size, 1, padding, dilation, groups, bias
				)
		self.f2f = torch.nn.Conv2d(
				in_channels, int(alpha * out_channels),
				kernel_size, 1, padding, dilation, groups, bias
				)

		# frequency modules
		self.MCSConv = MCSConv(in_channels)  # 输入和输出通道数一致
...
class SFIConv(nn.Module):
	def __init__(
		...
		self.f2f = None if alpha == 0 else torch.nn.Conv2d(
				int(alpha * in_channels), int(alpha * out_channels),
				kernel_size, 1, padding, dilation, math.ceil(alpha * groups), bias
				)
		self.f2s = None if alpha == 0 or alpha == 1 or self.is_dw else torch.nn.Conv2d(
				int(alpha * in_channels), out_channels - int(alpha * out_channels),
				kernel_size, 1, padding, dilation, groups, bias
				)
		self.s2f = None if alpha == 1 or alpha == 0 or self.is_dw else torch.nn.Conv2d(
				in_channels - int(alpha * in_channels), int(alpha * out_channels),
				kernel_size, 1, padding, dilation, groups, bias
				)
		self.s2s = None if alpha == 1 else torch.nn.Conv2d(
...
class LastSFIConv(nn.Module):
	def __init__(
		...
		self.s2s = torch.nn.Conv2d(
				in_channels - int(alpha * in_channels), out_channels,
				kernel_size, 1, padding, dilation, groups, bias
				)
		self.f2f = torch.nn.Conv2d(
				int(alpha * in_channels), out_channels,
				kernel_size, 1, padding, dilation, groups, bias
				)
...
class SFIConvBR(nn.Module):
	def __init__(
			self, in_channels, out_channels, kernel_size=(3, 3), alpha=0.5, stride=1, padding=1, dilation=1,
